...
test("documents every read-only tool and the purchase boundary", async () => {
  ...
  for (const tool of ["audit_public_website", "check_ai_crawler_policy", "get_crawler_watch_offer", "get_full_report_offer", "get_practice_radar_offer"]) assert.match(readme, new RegExp(tool));
  ...
  assert.match(readme, /Distribution version `1\.4\.0`/);
  assert.match(readme, /official registry[^\n]+version `1\.4\.0`/);
  assert.match(readme, /docker run --rm -i ghcr\.io\/unitedideas\/actablesite-mcp:1\.4\.0/);
